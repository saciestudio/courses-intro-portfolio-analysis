...
description : The history of portfolio returns contains valuable information about how much the investor can expect to gain or lose. This chapter introduces the R functionality to analyze the investment performance based on a statisical analysis of the portfolio returns. It includes graphical analysis and the calculation of performance statistics expressing average return, risk and risk-adjusted return over rolling estimation samples. This chapter concludes by an investigation in the non-normality of the return distribution aand provides tools to characterize the financial risks and rewards in a non-normal world. 
...
A picture is worth a thousand word. This is why most analyses of performance start by studying the time series plot of the value of the investment. For the S&P 500 this is shown in the upper plot for the period 1986 to current date. Each observation is the end-of-day value. Note the booms and busts in the value of this portfolio. Do you see why the 2000s are often called the lost decade of investing?
...
A good proxy for the risk free rate is the one-month Treasury bill rate. This Tbill rate is available in the console as the variable `rf`.  The S&P 500 portfolio returns are still available as `sp500_returns`.  
...
*** =instructions
- Compute the annualized risk free using the compound intrest formula and plot this series. 
...
*** =hint
If $R$ is the monthly interest rate. The yearly compounded intrest is given by $(1+R)^{12}$.
...
dates <- as.Date(ff[!is.na(rf),1], format="%Y%M")
rf<- xts(na.omit(rf),dates)
newdates <- seq(as.Date(time(rf))[2], length=nrow(rf), by="1 month") - 1
rf <-  xts( rf , order.by = newdates )
...
We have now computed the performance measure on each possible sample of a fixed size that can be obtained by rolling through time. Often we are interest in the performance over one particular subwindow.  Obtaining a subset of a time series in R is straightforward using the function the function [window](http://www.rdocumentation.org/packages/zoo/functions/window.zoo), having as first argument the return series that needs to be subset, then the start data and end date in the format `YYYY-MM-DD`.
...
Let us verify whether the [skewness](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/skewness) and [kurtosis](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/kurtosis) statistics confirm this graphical analysis. By default, the [kurtosis](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/kurtosis)  reports the excess kurtosis (that is, the kurtosis minus three). Let us then use the [jarque.bera.test](http://www.rdocumentation.org/packages/tseries/functions/jarque.bera.test) to see whether the null hypothesis of a normal distribution can be rejected at a significance level of 5%. 
 

*** =instructions
- Compute the skewness and excess kurtois of `sp500_daily` and `sp500_monthly`.
- Test for normality in the two series using the Jarque Bera test. 
...
plot.zoo(sp500)
library(PerformanceAnalytics)
sp500_daily <-  xts(CalculateReturns(sp500))[-1]
names(sp500_daily) <- "sp500_daily"

sp500 <- get.hist.quote(instrument="^GSPC",start=as.Date("1985-12-31"),end=Sys.Date(),quote="AdjClose",quiet=T,compression="m")
plot.zoo(sp500)
library(PerformanceAnalytics)
sp500_monthly <-  xts(CalculateReturns(sp500))[-1]
...
# jarque bera test 
jarque.bera.test(sp500_daily)
jarque.bera.test(sp500_monthly) 
```
...
When the return distribution is asymmetric, investors use additional risk measures that focus on describing the potential losses. One such measure of downside risk is the [SemiDeviation](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/DownsideDeviation.html) computing the standard deviation of the negative returns. A further popular measure is the so-called `Value-at-Risk` at a pre-definity loss probability of e.g. 5%. Loosely speaking, it corresponds to the 5% quantile of the return distribution, such that a more negative return can only happen with a probability of 5%. The expected shortfall at 5% is the average return when the return is below this 5% quantile. 

Use the default implementation in the function [VaR](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/VaR.html) and [ES](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/ES.html) to compute those downside risk estimates for the monthly return series `sp500_monthly`, and verify that they match with what you would expect from the histogram. 

*** =instructions
- Estimate the 2.5% and 5% value-at-risk of a monthly investment in S&P 500 returns. 
- Estimate the 2.5% and 5% expected shortfall of a monthly investment in S&P 500 returns.
...
Buy low sell high. but what if we bought high sold low?Monthly When do you step in ... timing
The use of the [table.Drawdowns](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/table.Drawdowns) function in PerformanceAnalytics.
