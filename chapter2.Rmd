---
title       : Interpreting portfolio performance 
...
These daily prices are available in your workspace as the variable `sp500`. It is of the [xts](http://www.rdocumentation.org/packages/xts/) time series class, implying, among others, that the observations are time-stamped.  Our interest is in describing the monthly performance. To do so, we first need to aggregate the daily price series to end-of-month prices. We will then compute the monthly returns and visualize them in an attractive table.  
...
*** =sample_code
```{r}
# The package PerformanceAnalytics and xts are pre-loaded
# The variable sp500 is pre-loaded
...
We only scratched at the surface of possible portfolio performance measures. Check the documentation in PerformanceAnalytics to find out which of the following statements are false. 

*** =instructions
- Like the [SortinoRatio](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/SortinoRatio), the [AdjustedSharpeRatio](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/AdjustedSharpeRatio) is an alternative for the Sharpe ratio that takes ing into account the non-normality of the return distribution when evaluating the reward received in terms of units of risk. - The larger the [TrackingError](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/TrackingError) of the portfolio returns compared to the returns of a benchmark strategy, the higher the risk that the returns on the portfolio strategy will deviate from the benchmark returns. 
