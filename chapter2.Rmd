...
*** =video_hls
//videos.datacamp.com/transcoded/985_portfolio_analysis/v1/hls-ch2_1.master.m3u8
...
*** =video_hls
//videos.datacamp.com/transcoded/985_portfolio_analysis/v1/hls-ch2_2.master.m3u8
...
Performance evaluation involves comparing your investment choicesq with an alternative investment choice. Most commonly used is comparison with an **(almost)** risk free asset, such as the US Treasury Bill. The return from a US Treasury Bill is known as a risk free rate, this is because Treasury Bills (T-Bills) are are guaranteed a future return, and are backed by the US Government. 
Do you recall the Sharpe Ratio from the video? The Sharpe Ratio is an important metric that tells us the return-to-volatility ratio. The Sharpe ratio is calculated by taking the mean of excess returns (returns - risk free rate), divided by the volatility of the returns. 

In this exercise you will be asked to annualize the risk free rate, this is done using the compound interest formula. The yearly compounded  interest rate is given by $(1+y)^{12}$. The annual rate is used to estimate a yearly return and is very useful for forecasting. 
...
The probabilistic view of portfolio performance evaluation considers that the time series of returns is driven by an underlying process.Performance evaluation aims at inferring future returns from past returns. Perfomance evaluation attemps to answer questions such as: 
  - (i) What is on average the value of a return that will be generated?
  - (ii) What will be the standard deviation of the returns?

It is important to remember that when trying to aswer those questions, you as an investor, expose yourself to estimation errors. Investors will never know exactly what the underlying processes are, they can only make a guess about them from past returns. This is the limitation of any estimation approach. It is almost sure that the estimates of the mean and volatility will be different from the true mean and standard deviation.

In this exercise, you will begin to familiarize yourself with uncertainty in estimation. You will do this by drawing random samples from a normal distribution with a mean of 5%, and a standard deviation of 10%. You will see how uncertainty is reduced when the sample size grows. We will use the `rnorm()` function with arguments `"n", "mean", "sd"` to take a sample from the normal distribution.
...
*** =video_hls
//videos.datacamp.com/transcoded/985_portfolio_analysis/v1/hls-ch2_3.master.m3u8
...
In previous exercises you have already familiarized yourself with the `Return.annualized` and `StdDev.annualized` functions. In this exercise you will use the function `SharpeRatio.annualized` to calculate the annualized Sharpe Ratio. This function takes the arguments `"R"`, and `"Rf"`. The `"R"` argument takes an xts, vector, matrix, data.frame, timeSeries or zoo object of asset returns. The `"Rf"` argument takes a risk-free rate in the same period of your returns. For this example you can use the object `rf` to fulfill the `"Rf"` argument. By default `SharpeRatio.annualized` geometric aggregation.
...
- Plot the rolling 12-month estimates of the annualized mean. Plot the unconditional long run estimate of the annualized return (`returns_ann`) as a horizontal references.
- Plot the rolling 12-month estimates of the annualized volatility of the S&P 500 returns. Plot the unconditional long run estimate of the annualized standard deviation (`sd_ann`) as a horizontal reference.
- Plot the rolling 12-month estimates of the annualized Sharpe Ratio of the S&P 500 returns. Plot the unconditional long run estimate of the annualized Sharpe Ratio (`sharpe_ann`) as a horizontal reference.
...
To the right are two plots of rolling annualized volatility.
...
- Plot histograms of returns in those two years using the function `chart.Histogram`. Set the argument `methods = c("add.density", "add.normal")` to visualize the non-parametric estimate of the density and the density under and assumed normal distribution. 
...
*** =video_hls
//videos.datacamp.com/transcoded/985_portfolio_analysis/v1/hls-ch2_4.master.m3u8
...
In this exercise you will verify whether the `skewness` and `kurtosis` statistics confirm the graphical analysis in the plot environment. By default, the `kurtosis` reports the excess kurtosis (that is, the kurtosis minus three). 
...
*** =hint
Use the functions `skewness` and `kurtosis`.
...
When the return distribution is asymmetric, investors use additional risk measures that focus on describing the potential losses. One such measure of downside risk is the `SemiDeviation`. The `SemiDeviation` is the calculation of the variability of returns below the mean return.

Another more popular measure is the so-called *Value-at-Risk* (or VaR). Value-at-Risk has three major components: pre-defined loss amount (loss percentage), a confidence level, and a time period. Loosely speaking, the VaR corresponds to the 5% quantile of the return distribution, meaning that a more negative return can only happen with a probability of 5%. For example you might ask:"what is the largest loss I could potentially take within the next quarter, with 95% confidence?" 
...
*** =hint
Drawdowns can be visualized using `chart.Drawdown`. 
