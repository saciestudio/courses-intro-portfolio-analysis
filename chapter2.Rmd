...
description : The history of portfolio returns contains valuable information on how much the investor can expect to gain of lose. In the first chapter, we used time series plots to visualize the past prices and returns. The advantage of the time series plot is that it shows the dependence over time. In this chapter, we will first ignore the time series properties and pool all the observations in one sample for which the we make use of the histogram and performance statistics to extract the relevant information embedded in the portfolio returns. These statistics are useful to describe the average performance over long investment periods. But, for active investors and risk managers, the key question is to predict the distribution of returns over the next period. For doing this, it is important to take the time series dependence into account. The practioner approach for doing this is to use rolling window analysis and comput the performance measures (mean, volatility, Sharpe ratio) on rolling samples of e.g. six months. This chapter concludes by an investigation in the non-normality of the return series of investments and provides tools to characterize the financial risks and rewards that that takes this non-normality into account. 
...
*** =instructions
- Print the annualized mean
- Print the annualized standard deviation
