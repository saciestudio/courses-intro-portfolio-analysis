...
The choice of investment matters, even when the underlying risky asset are similar. To illustrate this, let's download the daily adjusted close price history for the Coca-Cola company and Pepsi Cola and compare in a time series plot the value evolution of one dollar invested in each of them. We see that over the period 2004-2016 an investment in PepsiCola nearly trippled the initial investment value while a one dollar investment in the Coca-Cola Company's share has an end value of 2.38.

Modify the R script in order to compare, over the period starting on New Year's day of 2010 and ending on today's date, the value evolution of a one dollar investment in Apple (ticker `AAPL`) and IBM (ticker `IBM`)). 

*** =instructions
- Understand the steps in the given R code.
- Replace the ticker names and interpret the results. 
...
KO <- get.hist.quote(instrument="KO",start=as.Date("2003-12-31"),end=as.Date("2015-12-31"),quote="AdjClose",quiet=T)
PEP <- get.hist.quote(instrument="PEP",start=as.Date("2003-12-31"),end=as.Date("2015-12-31"),quote="AdjClose",quiet=T)
# 
...
# download adjusted close prices (that is corrected for dividend payments and stock splits)
# dates have the format "YYYY-MM-DD"
AAPL <- get.hist.quote(instrument="KO",start=as.Date("2003-12-31"),end=Sys.Date(),quote="AdjClose",quiet=T)
MSFT <-  
# 
AAPL <- AAPL/as.numeric(AAPL[1])
PEP <- PEP/as.numeric(PEP[1])
#
plot.zoo(  , ylim=c( min(c(min(AAPL),min(MSFT))) ,  max(c(max(AAPL),max(MSFT))) ) , ylab="")
lines(MSFT,col="red", )
legend("topleft", legend=c("Apple","Microsoft") , col=c("black","red") , lwd = 1 )
```
...
# download adjusted close prices (that is corrected for dividend payments and stock splits)
# dates have the format "YYYY-MM-DD"
AAPL <- get.hist.quote(instrument="AAPL",start=as.Date("2010-01-01"),end=Sys.Date(),quote="AdjClose",quiet=T)
MSFT <- get.hist.quote(instrument="MSFT",start=as.Date("2010-01-01"),end=Sys.Date(),quote="AdjClose",quiet=T)
# 
AAPL <- AAPL/as.numeric(AAPL[1])
MSFT <- MSFT/as.numeric(MSFT[1])
#
plot.zoo(AAPL, ylim=c( min(c(min(AAPL),min(MSFT))) ,  max(c(max(AAPL),max(MSFT))) ) , ylab="")
lines(MSFT,col="red",)
legend("topleft", legend=c("Apple","Microsoft") , col=c("black","red") , lwd = 1 )
```
...
An investor has  4000 USD invested in equities, 4000 USD in bonds and 2000 USD in commodities. Compute the weights as the proportion invested in each of those three assets. In order to compute the corresponding portfolio weights, define the vector `values` as the vector holding these three values. Then define the vector `weights` as the vector `values` divided by the total value (obtained by summing over the component values using the function `sum`).
...
Make use of the `head(prices,1)` and `tail(prices,1)` functions to print the first and last rows of `prices` and verify that the price data runs from December 1990 to December 2015. Then compute the monthly returns using the R command `Return.calculate(prices, method="simple")`. Print the first 6 rows of returns and verify that the first row consists of NA's. Remove the first row from returns using `returns <- returns[(-1),]`. 


*** =instructions
- Print the first and last row of `prices`
- Define `returns` as the multivariate return series 
