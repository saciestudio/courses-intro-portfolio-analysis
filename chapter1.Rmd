...
The time series plot shows you the value evolution of one dollar invested in each of them. As an exercise, plot the time series showing the relative value of an investment in the Coca Cola company, compared to the value of an investment in PepsiCo. To do this exercise, you can us the corresponding price series, available as the variables `ko` and `pep` in your R environment. 
...
Let us explore this functionality in the case of a portfolio invested in the Apple stock and Microsoft stock, for which the prices are available in the variables `aapl` and `msft` in the R environment. The target weights are the same for each rebalancing data. We call this the constant mix portfolio allocation strategy.   
