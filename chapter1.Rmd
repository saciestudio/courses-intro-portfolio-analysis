...
As an illustration, I plot the following absolute differences in portfolio return between a less frequent rebalanced portfolio and the  daily rebalanced portfolio. To which R command corresponds the first plot?
...
weights <- c(0.5,0.5)
# compute portfolio return for each date
par(mfrow=c(3,1),mar=c(3,2,2,2),cex.axis=1.3)
z1 = abs( Return.portfolio(returns, weights) - Return.portfolio(returns, weights,rebalance_on="days"))
plot.zoo( abs( Return.portfolio(returns, weights,rebalance_on="years") - Return.portfolio(returns, weights,rebalance_on="days")),ylab="",xlab="",ylim=c(0,max(z1)))
plot.zoo( abs( Return.portfolio(returns, weights) - Return.portfolio(returns, weights,rebalance_on="days")),ylab="",xlab="",ylim=c(0,max(z1)))
plot.zoo( abs( Return.portfolio(returns, weights,rebalance_on="months") - Return.portfolio(returns, weights,rebalance_on="days")),ylab="",xlab="",ylim=c(0,max(z1))) 
```
