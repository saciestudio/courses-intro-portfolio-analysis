...
- Use the function [Return.portfolio](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/Return.portfolio) with as first argument `prices` and second argument `weights` to compute for each date the return of a portfolio for which the weights on each date or set to `weights`.
...
weights <- c(0.5,0.5)
# compute portfolio return for each date
preturns <- Return.portfolio(returns, weights)
# plot the time series of portfolio returns
plot.zoo(preturns)
```
...
```
--- type:VideoExercise lang:r xp:50 skills:1
## Multiperiod returns and a sneak preview to the road ahead
