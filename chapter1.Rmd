...
*** =instructions
- The price series are available in the R environment as the variables `KO` and `IBM` 
- Define `KOvsIBM` as the ratio between `KO` and `IBM`
- Use `plot.zoo` to visualize the daily variation in this ratio 
- Note that, when the value of the ratio increases, `KO` outperforms `IBM` and vice versa
- Note also that, When the value of the ratio is larger than one, the cumulative performance of `KO` since Jan 2003 is higher than that of `IBM`. Thereofre, as a reference line, use `abline` to include a horizontal line at `h=1`.
...
The choice of investment matters, even when the underlying risky assets are similar. As a second example, let us now consider the stock price of Apple and Microsoft from Januari 2006 till today. 

The time series plot shows you the value evolution of one dollar invested in each of them. In addition, it shows the value evolution for an investor who invested, in January 2003, 50 cents in Apple and 50 cents in Microsoft. Since the investor does not change the portfolio allocation, we call this a buy and hold allocation. At the other extreme, one could consider the objective of investing, at the close of each day, half of the investor's wealth in the shares of Microsoft and half in the shares of Apple. We say that this portfolio is daily rebalanced.  
