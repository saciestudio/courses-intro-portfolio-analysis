...
# Verify that it matches with the percentage change in the portfolio value
all.equal( preturn   , (sum(fin.values)-sum(in.values))/sum(in.values) )
```
...
*** =instructions
- Load this two packages in your R session.
...
*** =instructions
- Load in the libraries `xts` and `PerformanceAnalytics`.
...
- In order to see how a one dollar investment evolved over that month, print `cumprod(1+preturns["2008-09"])`. Its $k$th observation  corresponds to the chain $1*(1+R_1)*(1+R_2)*\ldots*(1+R_K),$ with $R_i$ the $i$ th return of that month
