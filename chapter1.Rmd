...
*** =hint
[`plot.zoo()`](http://www.rdocumentation.org/packages/zoo/functions/plot.zoo) only needs one argument, ko_pep.
...
#5th instruction
test_object("preturns", incorrect_msg = "Make sure you calculated the portfolio return correctly!")
...
In the previous exercise you explored the functionality of the `Return.portfolio()` function and created portfolios using two strategies. However, by setting the argument `verbose = TRUE` in `Return.portfolio()` you can create a list of beginning of period and end of period, weights and values in addition to the portfolio returns, and contributions.
...
The next chapter is devoted to performance evaluation. But before you dive into that, it would do you well to look at a bit of investment history. In this exercise you will focus on the evolution of returns during September, 2008. This was an exception month for investors. However, the bankruptcy of Lehman Brother on September 15th was an important turning point. Complete the exercise for a closer look.
...
To maky you hungry for more, look at the nice output tables and plots printed in the workspace. This is what we will be doing in the next chapter, leveragin mainly the functionality in the package [PerformanceAnalytics](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/PerformanceAnalytics-package). 

Which of the following functions is not in the package [PerformanceAnalytics](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/PerformanceAnalytics-package). 
...
- [SharpeRatio](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/SharpeRatio).  Sharpe and Tobin. 
