...
## From simple to gross returns and the asymmetric impact of gains and losses  
...
Suppose the investor now has a horizon of two periods and in the first period, she makes a 10% return, but in the second period a loss of 5%. Verify in the R console that the end value of the investment equals $1.045$ (obtained by multipying the end-value of 1.1 after period one with the gross return of 0.95 of the second period).
...
The default behavior of the function Return.portfolio is that is does not rebalance, which corresponds to a buy-and-hold investment. 
...
z0 <-  Return.portfolio(returns, weights,rebalance_on="days")
z1 <- abs( Return.portfolio(returns, weights,rebalance_on="years") -as.numeric(z0))
z2 <- abs( Return.portfolio(returns, weights) -z0)
z3 <- abs( Return.portfolio(returns, weights,rebalance_on="months") - z0)
plot.zoo( z1 , ylab="",xlab="",ylim=c(0,max(z2)))
plot.zoo( z2 , ylab="",xlab="",ylim=c(0,max(z2)))
plot.zoo( z3 , ylab="",xlab="",ylim=c(0,max(z2)) )
#print(max(z2))
```
...
```
--- type:VideoExercise lang:r xp:50 skills:1
## Sneak preview of the fin and fun ahead
