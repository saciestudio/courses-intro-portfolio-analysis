...
Welcome to Introduction to Portfolio Analysis! In the last video you were given a brief overview of some of the key concepts in portfolio analysis. The goal of every stock investor is to make profits while reducing the amount of risk involved. Investors create strategies to reduce these risks.
...
*** =hint
If you're struggling, try rewatching the video!
...
*** =hint
Having a large quantity of capital isn't always necessary to begin investing!
...
The equally weighted portfolio is one of the most used portfolios. Suppose the portfolio is equally invested in $N$ assets. Let `N <- 100`. Use the R console to determine which of the following commands creates an equally weighted portfolio. 
...
In a market capitalization weighted portfolio, the weights are given by the individual assets' market capitalizations divided by the sum of the market capitalizations of all assets. A typical example is the S&P 500 invested in the 500 largest companies listed on the US stock exchanges (NYSE, Nasdaq).  Note that dividing by the total value of the characteristic across all assets guarantees that the portfolio weights sum up to unity. 
...
#3rd instruction
test_student_typed("summary(weights)", not_typed_msg = "Make sure to plot a summary of `weights`!", not_typed_msg = "Make sure you print a summary of the weights!")
...
Below you find three commands to compute the portfolio return of an equally weighted portfolio of `N <- 100` assets with return vector `returns`. Using your R console, determine which of the following commands creates your portfolio returns. 

*** =instructions
- `preturn <- mean(returns / N)`
- `preturn <- mean(returns)`
- `preturn <- rep(mean(returns), N / returns)`
- None of the above.
...
The gross returns over two periods is obtained similarly. Let *$R_1$* be the return in the first period and *$R_2$* the return in the second period. Then the end-value of a 1 USD investment is   $(1 +$*$R_1$*$) * (1 + $*$R_2$*$)$.
...
test_student_typed("tail(prices)", incorrect_msg = "Oops! Be sure to look at the tail of `prices`!")

#4th instruction
test_object("returns", incorrect_msg = "Did you provide the correct object to `Return.calculate()`?", undefined_msg = "You need to create an object of returns!", index = 1)

#5th instruction
test_output_contains("head(returns)", incorrect_msg = "It looks like you didn't print the head of your returns!")

#6th instruction
test_object("returns", index = 2)
...
- Create a portfolio where you rebalance your weights monthly. Use `Return.portfolio()` with the argument `rebalance_on = “months”`. Call this `pf_monthly`.
...
pf_bh <- Return.portfolio(___, weights = ___)
...
#2
test_obect("pf_bh", incorrect_msg = "You should set your weights argument equal to `weights` and `verbose = TRUE`!", undefined_msg = "Oops! You forgot to create a portfolio using a buy and hold technique.")

#3
test_obect("pf_rebal", incorrect_msg = "You should set your `rebalance_on` argument to `months`, and `verbose = TRUE`!" undefined_msg = "Oops! You forgot to create a portfolio using a monthly rebalancing technique.")
...
-- WE WOULD LIKE TO REMOVE THIS EXERCISE, because compound returns are not introduced previously and do not occur anymore in the remaindder of this course (except for annualizing, (1+i)^12). Also, the numbers seem to be wriong.--

You have now plotted the daily return series. In practice, portfolio managers rarely communicate their daily returns, but focus on the total (compound) performance over a longer period, such as a month or a year. 
Monthly returns can be obtained by using the formula of compound returns. A compound return at the $k$-th period is given by the cumulative product of the returns for periods 1 to $k$, which is given by the chain $(1 +$*$R_1$*$) * (1 + $*$R_2$*$) * \ldots * (1+$*$R_K$*$)$. To get to the return *over* a certain period (let's say the period 1 to $K$), you have to substract the compound return at the $K$-th period  


The next chapter is devoted to performance evaluation. But before you dive into that, it would do you well to look at a bit of investment history, using a daily rebalanced portfolio containing the Apple and Microsoft stocks used previously.

In this exercise you will focus on the evolution of portfolio returns in September 2008. This was an exceptional month for investors. However, the bankruptcy of Lehman Brother on September 15th was an important turning point. Complete the exercise for a closer look.

*** =instructions
- Print the daily returns in September 2008. This can be done using `preturns["YYYY-MM"]`.
- In order to see how a one dollar investment evolved over that month, print `cumprod(1+preturns["2008-09"])`. Its $k$-th observation  corresponds to the chain $(1 +$*$R_1$*$) * (1 + $*$R_2$*$) * \ldots * (1+$*$R_K$*$)$ with *$R_i$* the $i$-th return of that month.
...
*** =sample_code
```{r}
# returns is pre-loaded, and the formula for a daily rebalanced equally weighted portfolio is given below
preturns <- Return.portfolio(returns, weights, rebalance_on= "days")
