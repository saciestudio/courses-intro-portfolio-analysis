...
- You want to see how the portfolio return changes with a changing weight. Create an object `preturns` that equals the sum of `weights` times `returns_equities`, and `1-weights` times `returns_bonds`.
...
*** =hint
To calculate your portfolio returns, tour code should look like: `preturns <- weight * returns_equities + (1 - weight) * returns_bonds`.
...
- Compute and visualize the correlation again, using [chart.Correlation()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/chart.Correlation) with `returns` as the argument. 
- Commpute the rolling 24-month estimates of the bond-equity correlation using the function `chart.RollingCorrelation()`. Note that `returns_bonds` is the so-called "benchmark asset", and should be included as the second argument.
...
*** =instructions
- Create a vector of weights that leads to 40% invested in equities, 40% in bonds, 10% in real estate and 10% in commodities. Call this vector `weights`.
...
--- type:NormalExercise lang:r xp:100 skills:1 key:13b6fbf2b7
...
The matrices you created in the last exercise are loaded in your workspace as `w`, `mu`, and `sigma`. In addition the return series created from using `Return.portfolio()` is loaded as `preturns`. You will compare this to the matrices.
...
*** =sct
```{r}
test_output_contains("12 * (t(w) %*% mu)", incorrect_msg = "Oops! It looks like you need to calculate the annualized portfolio returns using matrices.")
test_output_contains("sqrt(12) * sqrt(t(w) %*% sigma %*% w)", incorrect_msg = "Make sure you calculate the annualized volatility using matrices!")
test_output_contains("12 * mean(preturns)", incorrect_msg = "Be sure to calculate annualized portfolio returns using `preturns`!")
test_output_contains("sqrt(12) * sd(preturns)", incorrect_msg = "Did you calculate the annualized volatility using `preturns`?")
