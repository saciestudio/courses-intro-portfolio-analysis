...
- Plot the value of the Sharpe ratio as a function of the equity weight in the portfolio, and indicate with a vertical line the value of the equities weight that maximizes the portfolio Sharpe ratio. 
...
- Merge `return_equities`, and `returns_bonds` using [merge()](http://www.rdocumentation.org/packages/base/functions/merge). Call this `returns`.
...
- Commpute the rolling 24-month estimates of the bond-equity correlation using the function [chart.RollingCorrelation()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/chart.RollingCorrelation), use the arguments `R`, and `width`. 

*** =hint
[chart.RollingCorrelation()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/chart.RollingCorrelation) uses two time-series for the argument `R`, and the `width` is the number of months.
...
#2nd
test_function("cor", c("x", "y"), not_called_msg not_called_msg = "Be sure to find the correlation of returns!")
...
```{r}
#1st
test_function("apply", c("X", "MARGIN", "FUN"), not_called_msg not_called_msg = "Be sure to calculate column-wise means!", incorrect_msg = "Did you specify the correct function?", index = 1)
...
test_output_contains("abline(h = 0, lty = 3", "He! Did you change our code?")
...
success_msg("Well done! These matrices can help you in your calculation, and are also a good reference to see the relationship between your assets.")
```
...
For this last exercise you will calculate the risk contributions for a portfolio that is invested *40% in equities, 40% in bonds, 10% in real estate, and 10% in commodities*. To do so, you will use the function [StdDev()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/StdDev). The [StdDev()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/StdDev) function creates a list of the assets' standard deviation (`$StdDev`), their risk contribution (`$contribution`), and their percent risk contribution (`$pct_contrib_StdDev`). 

You will be using three arguments in the [StdDev()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/StdDev) function to do this calculation. The first is `R`, a vector, matrix, data frame, time series or zoo object of returns. The second is `portfolio_method`, which you will be setting to `"component"`, and the third is `weights`.

The object `returns` is loaded in your workspace.

*** =instructions
- Create a vector of the portfolio weights. 
- Calculate your volatility budget using [StdDev()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/StdDev). Set `portfiol_method = "component" and `weights` equal to the vector of weights. Call this `vol_budget`.
...
*** =hint
If you're struggling with the [StdDev()](http://www.rdocumentation.org/packages/PerformanceAnalytics/functions/StdDev) function, look at the exercise description. Everything you need is there!
