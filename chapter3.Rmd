...
*** =instructions
- Cast the vector of estimated means returns into a column-matrix using `as.matrix( )`.
...
*** =sample_code
```{r}
# mu is the column-matrix of estimated expected returns
vmeans <- apply(returns,2,"mean")
mu <- 
# sigma is the estimated covariance matrix
sigma <- cov(returns)
# matrix of weights
w <- as.matrix(  c(0.4,0.4,0.1,0.1)  )
# transsposed matrix
tw <- 
# portfolio mean
