...
We have seen that the portfolio variance depends on the correlation between the asset returns. Keeping constant the volatilities, we have that the lower the correlation is, the more diversification there is between the assets and thus the lower the portfolio variance.  

Let us now compute the correlation between equity returns and bond returns. We will do this first taking all returns in the sample. However, just like volatilities, these correlations are dynamic, as can be seen from the time series of correlations that we obtain when estimating the correlations over rolling samples. 
