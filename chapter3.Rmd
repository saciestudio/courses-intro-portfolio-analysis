...
- The portfolio mean return equals the sum of the component average returns (that is, `mean(returns_6040)==0.6*mean(returns_equities)+0.4*mean(returns_bonds)`). We call this the 
- For a portfolio that yields more than the risk-free rate, the Sharpe ratio is higher than the the sum of the component Sharpe ratios (that is, `mean(returns_6040)/sd(returns_6040) > 0.6*mean(returns_equities)/sd(returns_equities)
     +0.4*mean(returns_bonds)/sd(returns_bonds)`. 
