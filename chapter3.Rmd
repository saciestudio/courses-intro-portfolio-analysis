...
The correlation is an important number, since the lower the correlation is, the more the risks in the indvidual investments offset each other. Those gains in diversification then lead to a lower portfolio volatility, as shown in the video. 

Measuring is knowing. et us now compute the correlation between equity returns and bond returns. We will do this first taking all returns in the sample. However, just like volatilities, these correlations are dynamic, as we will see from the time series of correlations that we obtain when estimating the correlation over rolling samples. 
