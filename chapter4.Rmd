...
- Create a sequence (`seq()`) of length 50, that begins at a risk free rate of 1% and ends at the maximum value of `stockmu` called `grid`.
- Initialize 2 empty vectors with the same length as `grid`, where you will store the portfolio means and standard deviations. Call these `vpm` and `vpsd`. 
...
test_output_contains("for(i in 1:length(grid)) {
  optim <- portfolio.optim(x = returns , pm = grid[i])
  vpm[i] <- optim$pm
  vpsd[i] <- optim$ps
  mweights[i, ] <- optim$pw
...
*** =hint
In the financial market, not all risks are compensated with a higher return.
*** =pre_exercise_code
```{r}
library(xts)
library(PerformanceAnalytics)
...
*** =instructions
- Create `weights_minvar` as the row in `mweights` corresponding to the portfolio with the minimum standard deviation in `vpsd`. 
- Calculate the Sharpe ratio of portfolio returns when the risk free rate is *0.75%*. Call this `vsr`.
- Create `weights_max_sr` as the row in `mweights` corresponding to the portfolio with the maximum sharpe ratio in `vsr`.
