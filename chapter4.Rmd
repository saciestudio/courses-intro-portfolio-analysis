...
- Create three empty vectors with the same length as `grid`, where you will store the portfolio means, standard deviations, and weights. Call these `vpm`, `vpsd`, `mweights` respectively. Note that the first observations of `vpm`, and `vpsd` will remain `NA` because returns cannot be calculated on the initial observation.
...
- Use `rbind()` to merge the weights of each portfolio to `mweights` to build your weight matrix.
