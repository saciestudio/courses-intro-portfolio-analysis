...
Recall that if we compute the means column by column using `colMeans(returns)`, or `apply(returns,2,"mean")`, you then obtain the average return per asset. In this exercise, you will be computing the mean per row. By doing so, you obtain the time series of returns for the equally weighted portfolio.
...
- Investigate the dimensions of `ew_preturns`.
...
You will create a portfolio that is optimized for a target return that equals the average value of the return series `returns`. Then you will create a portfolio that has a target return that is 10% higher than the mean return series, and calculate the percent change in risk.
