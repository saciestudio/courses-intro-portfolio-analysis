...
- Create a vector of row means of `returns` using the function [rowMeans()](http://www.rdocumentation.org/packages/base/functions/colSums). Assign this to `row_means`. Note that you could have used `apply()` here as well.
- Plot `row_means` using `plot.zoo()`.
- Create a correlation matrix of the returns called `z`.
- Plot a histogram of the correlation matrix, `z`.
...
#5
test_function("hist", "x", incorrect_msg = "Oops! Have you plotted the lower triangle of your correlation matrix properly?")
