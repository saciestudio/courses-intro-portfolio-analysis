...
Recall that, if we compute the means column by column using `colMeans(returns)` or `apply(returns,2,"mean")`, we then obtain the average return per asset. In this exercise, we will be computing the mean per row. By doing so, we obtain the time series of returns for the equally weighted portfolio.
...
- Create a vector of row means of `returns` using the function [rowMeans()](http://www.rdocumentation.org/packages/base/functions/colSums). Assign this to `ew_preturns`. Note that you could have used `apply()` here as well.
- Investigate the dimension of `ew_preturns`.
...
#2
test_object("ew_preturns", undefined_msg = "Did you calculate the row means of `returns`?", incorrect_msg = "Did you calculate the row means of `returns` correctly?")

#3
test_function("dim", "x", not_called_msg = "Did you have a look at the dimension of `ew_preturns`?")

#4
test_function("xts", c("x", "order.by"), not_called_msg = "Did you convert the numeric vector `ew_preturns` back to an xts object?")
...
- Use [barplot](http://www.rdocumentation.org/packages/raster/versions/2.5-2/topics/barplot) to visualize the distribution of `opt_weights`.
...
test_object("opt", undefined_msg = "Remember to create your optimized portfolio!", incorrect_msg = "Be sure to supply `returns` to `portfolio.optim()`!")
...
#4
test_object("opt_weights", undefined_msg = "You need to create your optimum weights object!", incorrect_msg = "Remember that optimum weights are greater than or equal to 1%!")

#5
test_function("barplot", "height", not_called_msg = "Remember to plot your optimum weights!", incorrect_msg = "Make sure you selected the correct values for your optimum weights.")

#5
test_output_contains("opt$pm")
test_output_contains("opt$ps")
...
You will create a portfolio that is optimized for a target return that hat equals the average value of the return series `returns`. Then you will create a portfolio that has a target return that is 10% higher than the mean return series, and calculate the percent change in risk.


*** =instructions
- Create a portfolio where the target return is the mean of `returns`. Store the output as the variable `pf_mean`.
...
#4
test_output_contains("(pf_10plus$ps - pf_mean$ps) / (pf_mean$ps)", incorrect_msg = "Make sure you calculate the percent increase in standard deviation!")
...
Remember from the previous exercise that the function `portfolio.optim()` allows you to set weight constraints within the
`reshigh` argument. `reshigh` requires a vector of maximum weights for each asset.
...
*** =hint
To compute the number of elements larger than 0.01 in a vector x, you could use `sum(x > 0.1)`
...
# Create an optimum portfolio with max weights of 100%
opt1 <- portfolio.optim(returns, reshigh = weights1)

# Create an optimum portfolio with max weights of 10%
opt2 <- portfolio.optim(returns, reshigh = weights2)

# Create an optimum portfolio with max weights of 5%
opt3 <- portfolio.optim(returns, reshigh = weights3)
...
```{r}
#1
test_object("max_weights1", undefined_msg = "Be sure you create the three weights vectors!", incorrect_msg = "Make sure that this is a vector where each value is equal to 1.")
test_object("max_weights2", undefined_msg = "Be sure you create the three weights vectors!", incorrect_msg = "Make sure that this is a vector where each value is equal to 0.1")
test_object("max_weights3", undefined_msg = "Be sure you create the three weights vectors!", incorrect_msg = "Make sure that this is a vector where each value is equal to 0.05")

#2
test_function("portfolio.optim", c("x", "reshigh"), not_called_msg = "Make sure to create a portfolio where the max weights are equal to 100%!", incorrect_msg = "Did you supply the correct weights vector to `opt1`?", index = 1)

#3
test_function("portfolio.optim", c("x", "reshigh"), not_called_msg = "Make sure to create a portfolio where the max weights are equal to 10%!", incorrect_msg = "Did you supply the correct weights vector to `opt2`?", index = 2)

#4
test_function("portfolio.optim", c("x", "reshigh"), not_called_msg = "Make sure to create a portfolio where the max weights are equal to 5%!", incorrect_msg = "Did you supply the correct weights vector to `opt3`?", index = 3)

#5
test_output_contains("sum(opt1$pw > .01)", incorrect_msg = "Make sure you count how many assets have a weight greater than 1% for `optim1`!")
test_output_contains("sum(opt2$pw > .01)", incorrect_msg = "Make sure you count how many assets have a weight greater than 1% for `optim2`!")
test_output_contains("sum(opt3$pw > .01)", incorrect_msg = "Make sure you count how many assets have a weight greater than 1% for `optim3`!")

#6
test_output_contains("opt1$ps")
test_output_contains("opt2$ps")
test_output_contains("opt3$ps")
...
As you have seen, one approach to compute the efficient frontier is to first define the grid of target returns and then find for each target return, the portfolio that has an expected return equal to the target return at the lowest possible variance. 

But what is a reasonble grid of target returns? You will set the maximum target return to the maximum average return of the stocks. Ideally you would set the minimum target return to the return of the minimum variance portfolio. Because you don't know this minimum variance portfolio return yet, you create a grid using the minimum of average returns from all stocks.

In this exercise you will use a for loop to calculate your grid of potential portfolio means, deviations, and weights. For this exercise, note that your for loop will begin on the second iteration because you cannot calculate returns when there have been no prior observations. 

*** =instructions
- Calculate the column means of `returns`, and call this `stockmu`.
- Create a sequence (`seq()`) of length 50, that begins at the minimum value of `stockmu` and ends at the maximum value of `stockmu` called `grid`.
- Initialize 2 empty vectors with the same length as `grid`, where you will store the portfolio means and standard deviations. Call these `vpm` and `vpsd` Note that the first observations of `vpm`, and `vpsd` will remain `NA` because returns cannot be calculated on the initial observation.
- Initialze an empty matrix of 50 rows, and 30 columns. Call this `mweights`. You can use the `matrix()` function to do so.
- Create a for loop that starts on the second value of `grid` and ends on the last. 
- The for loop should create a portfolio called `optim` using `returns` and with a target return of `grid`.
- With each iteration the for loop should fill the vectors `vpm`, `vpsd` with their respective values from `optim`.
- Store the portfolio weights line by line in `mweights`.

*** =hint
Use the argument `length.out = 50` to set the length of your sequence for `grid`.
...
# Create empty vectors to store means, deviations and your matrix of weights
...
test_object("vpm")
test_object("vpsd")
test_object("mweights")

test_output_contains("for(i in 2:length(grid)) {
  optim <- portfolio.optim(x = returns , pm = grid[i])
  vpm[i] <- optim$pm
  vpsd[i] <- optim$ps
  mweights[i, ] <- optim$pw
...
In the previous exercise you computed the efficient frontier from a grid of potential target returns. In this exercise you will interpret the chart produced from the info data you created in the previous exercise.

Look at the chart to your right and identify which of the following is true. 

*** =instructions
- The minimum variance portfolio has a monthly volatility of around 1.05
- The maximum return portfolio has a monthly volatility that is lower than the tangency portfolio.
- The portfolio with the greatest return for its given standard deviation is the tangency portfolio.
- None of the above are true.

*** =hint
The efficient frontier has only one point where return is greatest for its standard deviation.
...
for(i in 2:length(grid)) {
  optim <- portfolio.optim(x = returns , pm = grid[i])
  vpm <- append(vpm, optim$pm)
  vpsd <- append(vpsd, optim$ps)
  mweights <- rbind(mweights, optim$pw)
}
...
*** =instructions
- Calculate the returns of a portfolio with the estimation sample `returns_estim`, and where the weights are the weights of the optimized portfolio `pf_estim`. Call this `return_pf_estim`.
- Calculate the returns of a portoflio with the evalutation sample `returns_eval`, and where the weights are the weights of the optimized portfolio `pf_estim`. Call this `returns_pf_eval`.
- Use the function `table.AnnualizedReturns()` on `returns_pf_estim`.
- Use the function `table.AnnualizedReturns()` on `returns_pf_eval`.
...
```{r}
# sct code
test_function("Return.portfolio", c("R", "weights", "rebalance_on"), index = 1)
test_function("Return.portfolio", c("R", "weights", "rebalance_on"), inde = 2)
...
- Read my article on portfolio optimization with [DEoptim](https://journal.r-project.org/archive/2011-1/RJournal_2011-1_Ardia~et~al.pdf). 
*** =hint
...
```{r}
# sct code
success_msg("You have now finished Intro to Portoflio Analysis! Congratulations!")
```
