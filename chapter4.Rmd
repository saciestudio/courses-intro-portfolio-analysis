...
- Create three empty vectors where you will store the portfolio means, standard deviations, and weights. Call these `vpm`, `vpsd`, `mweights` respectively.
...
- With each iteration the for loop should fill the vectors `vpm`, `vpsd` with their respective values from `optim`. You can use the function [append()](http://www.rdocumentation.org/packages/base/functions/append) to do this.
- Use `rbind()` to merge the weights of each portfolio to `mweights`.
...
for (i in 2:length(grid)) {
  optim <- 
  vpm <- append(vpm, ___)
  vpsd <- append(vpsd, ___)
  mweights <- rbind(mweights, ___) 
}
```
