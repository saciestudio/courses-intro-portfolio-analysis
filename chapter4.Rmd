...
- Plot `row_means` using [plot.zoo()](http://www.rdocumentation.org/packages/zoo/functions/plot.zoo).
...
#3
test_function("plot.zoo", "x", not_called_msg = "Be sure to plot the row means!")
...
- Create a mean-variance efficient portfolio of monthly returns using [portfolio.optim()](http://www.rdocumentation.org/packages/tseries/functions/portfolio.optim), called `optim`. 
...
# Calculate the percent increase in standard deviation
(pf_10plus$ps - pf_mean$ps) / (pf_mean$ps) * 100
...
In this exercise you will create three portfolios with different maximum weight constraints. For this exercise it is important to know the output of the [portfolio.optim](http://www.rdocumentation.org/packages/tseries/functions/portfolio.optim) function. This function creates a list containing four components: (i) `$pw`: the portfolio weights, (ii) `$px`: the returns of the overall portfolio, (iii) `$pm`: the exepcted return portfolio, (iv) `$ps`: the standard deviation of the portfolio returns.
